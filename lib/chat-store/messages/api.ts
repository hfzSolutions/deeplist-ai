...
export async function getMessagesFromDb(
  ...
  const { data, error } = await supabase
    .from("messages")
    .select(
      "id, content, role, experimental_attachments, created_at, parts, message_group_id, model, agent_id, agents(id, name, avatar_url)"
    )
    .eq("chat_id", chatId)
  ...
  return data.map((message) => ({
    ...
    agent: message.agents
      ? {
          id: message.agents.id,
          name: message.agents.name,
          avatar_url: message.agents.avatar_url,
        }
      : null,
  }))
}
...
async function insertMessageToDb(chatId: string, message: MessageAISDK) {
  ...
  await supabase.from("messages").insert({
    ...
    agent_id: (message as any).agent_id || null,
  })
}
