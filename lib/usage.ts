...
// Model configuration is now fetched from database
import { SupabaseClient } from "@supabase/supabase-js"
import { createClient } from "@/lib/supabase/client"

const isProModel = async (modelId: string) => {
  const supabase = createClient()
  if (!supabase) return true // If no supabase, treat as pro model
  
  // Get free model IDs from database
...
export async function checkUsage(supabase: SupabaseClient, userId: string) {
  ...
  // Use authenticated user daily limit
