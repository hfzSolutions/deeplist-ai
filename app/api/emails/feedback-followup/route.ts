  ...
  try {
    ...
    if (!feedbackType || !feedbackId || !recipientEmail) {
      return NextResponse.json(
        { error: 'Missing required fields: feedbackType, feedbackId, recipientEmail' },
        { status: 400 }
      )
    }
    ...
    if (!['bug', 'feature', 'general', 'support'].includes(feedbackType)) {
      return NextResponse.json(
        { error: 'Invalid feedback type. Must be: bug, feature, general, or support' },
        { status: 400 }
      )
    }
    ...
    if (!snippet) {
      ...
      if (feedback?.message) {
        ...
        snippet = feedback.message.length > 100 
