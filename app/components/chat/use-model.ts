...
export function useModel({
  ...
  const [defaultModel, setDefaultModel] = useState<string>("openrouter:deepseek/deepseek-r1:free")
  ...
  // Calculate the effective model based on priority: chat model > agent model > first favorite model > default
  const getEffectiveModel = useCallback(() => {
    const firstFavoriteModel = user?.favorite_models?.[0]
    return currentChat?.model || selectedAgent?.model || firstFavoriteModel || defaultModel
  }, [currentChat?.model, selectedAgent?.model, user?.favorite_models, defaultModel])
